...
def apply_custom_css():
    ...
    <style>
        ...
        div[role="radiogroup"] label {
            ...
            transition: all 0.3s; min-height: 80px; display: flex; align-items: center;
            justify-content: center; font-weight: 600; color: var(--text-sub);
        }
        div[role="radiogroup"] label[data-checked="true"] {
            border: 2px solid transparent !important;
            background: linear-gradient(white, white) padding-box, var(--btn-gradient) border-box !important;
            color: var(--deepseek-blue) !important; transform: translateY(-4px);
        ...
        /* 6. 上传与按钮美化 */
        [data-testid="stFileUploader"] section { border-radius: 12px; background-color: white; border: 2px dashed #dbe0ea; padding: 1.5rem; }
        ...
        div.stButton > button {
            ...
            background: var(--btn-gradient); color: white; border: none; transition: all 0.3s ease;
...
def display_pretty_report(title, report_text, bg_color="#eef5ff"):
    ...
    parts = re.split(r'(分大区情况如下：|分经营部情况如下：|分客户情况如下：|重点关注/严重逾期客户情况如下：|分品种情况如下：)', report_text)
    header_text = parts[0]
    detail_text = "".join(parts[1:]) if len(parts) > 1 else ""
