...
# ============================================================================
# PART 1: 初始保证金处理逻辑 (XSchushi.txt / app.py 原有逻辑) - 严禁修改
# ============================================================================
...
def display_pretty_report(title, report_text, bg_color="#eef5ff"):
    ...
    # 尝试拆分：通常第一段是总结，后面是明细（"分大区情况如下：" 或 "分经营部情况如下："）
    parts = re.split(r'(分大区情况如下：|分经营部情况如下：)', report_text)
    
    header_text = parts[0]
    detail_text = ""
    if len(parts) > 1:
        detail_text = "".join(parts[1:]) # 把拆分掉的标题加回去
...
def main():
    ...
    with col_center:
        ...
        # --- 模块 2: 追加保证金处理 (新功能) ---
            ...
            # 使用 st.pills (胶囊按钮) 替代 radio，避免 CSS 冲突
            # selection_mode="single" 确保单选
            selection = st.pills(
                "选择报告生成范围", 
                region_options, 
                default="中粮贸易", 
            ...
            # 逻辑兜底：如果用户再次点击已选按钮导致取消选择（变为None），强制回弹到"中粮贸易"
            # 这样保证了“选中状态一直保持”的体验
