...
def core_data_update_modal():
    ...
    if pwd == "xuyingzhe":
        ...
        # --- 新增：下载现有客户关系清单 ---
        # 允许管理员在更新前先下载查看当前仓库中现存的客户关系清单
        @st.cache_data(ttl=60, show_spinner=False)
        def _fetch_existing_mapping():
            """从 GitHub 仓库拉取现有客户关系清单的字节内容（缓存60秒）"""
            try:
                ...
                contents = repo.get_contents("客户关系清单.xlsx")
                return contents.decoded_content
            except Exception:
                return None

        existing_bytes = _fetch_existing_mapping()
        if existing_bytes is not None:
            st.download_button(
                label="📥 下载现有客户关系清单 (备份)",
                data=existing_bytes,
                file_name="客户关系清单.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
        else:
            st.info("ℹ️ 暂未检测到云端已有清单，请直接上传新清单。")

        new_mapping_file = st.file_uploader("上传新清单（用于更新替换）", type=['xlsx'], label_visibility="collapsed")
...
<p>本终端是专为风控条线量身定制的自动化数据中枢。我们将繁杂、冗长的传统多源报表处理，重塑为极简的"一键式"智能流水线，为您提供高效、精准的风险洞察体验。</p>

<hr style="border: 0; border-top: 1px solid var(--ac-wood); margin: 15px 0;">

<h4 style="color: var(--ac-green);">🆚 破局传统：为什么 Take It Easy 是更优解？</h4>
<p>传统商业智能 (BI) 与 Power Query 等工具在泛用型数据处理上固然强大，但在应对风控条线高频、定制化的报表需求时，往往面临"大而全却不精"的局限。Take It Easy 致力于解决传统工具的以下痛点，提供不可替代的专属体验：</p>
<ul style="padding-left: 20px;">
<li><b>打通数据汇报的"最后一公里" (End-to-End Output)：</b> 传统 BI 工具的终点往往是"可视化看板"，用户看完数据后仍需手动截图拼凑 Word。而 Take It Easy 能够穿透底层数据，直接封装为<b>排版就绪、带标准通报话术的 Word 催收函与风控简报</b>，彻底消除"看数据"与"写报告"的割裂感。</li>
<li><b>零代码的专家级风控引擎 (Hyper-Customization)：</b> 使用 Power Query 往往要求人员具备编写 M 函数或 DAX 的能力。Take It Easy 将特化的复杂业务逻辑（如：智能感知 1-49 吨与大宗标的的单位跃升、特定的百分比映射规则）<b>黑盒化封装</b>。它不仅是清洗工具，更是开箱即用的"数字风控专家"。</li>
<li><b>跨平台排版防火墙 (MS Word / WPS 像素级完美兼容)：</b> 传统自动化脚本生成的文档极易因为办公软件底层解析差异，在 WPS 中出现列宽缩水、自适应变形等排版崩塌问题。Take It Easy 采用底层 XML 深度锚定技术，确保输出报告在微软 MS Word 与国产 WPS Office 中<b>实现 100% 零误差、像素级一致渲染</b>。</li>
<li><b>极简架构与"阅后即焚"的合规保障 (Ephemeral Security)：</b> 传统工具往往需要安装沉重客户端，极易在本地留下数据缓存。Take It Easy 采用极轻量架构，即开即用；且计算全过程在内存中流转，<b>阅后即焚、零数据落盘</b>，完美规避客户数据泄露的合规风险。</li>
</ul>
...
def main():
    ...
    # 全局结果缓存：处理完成后存入 session_state，跨 rerun 保持状态
    # 仅当用户主动刷新浏览器或关闭标签页时清除
                ...
                if uploaded_file:
                    with st.spinner("正在智能判断报告层级并生成专属报告..."):
                        ...
                        if output_file:
                            ...
                            st.session_state.module_results["add_margin"] = {
                            ...
                            st.session_state.module_results["add_margin"] = None
                            st.error("处理失败")
                ...
                if not uploaded_files:
                    ...
                    with st.spinner("正在智能识别文件类型并生成报告中..."):
                        ...
                        if excel_io:
                            ...
                            st.session_state.module_results["overdue_sales"] = {
                                "excel_io": excel_io,
                                "word_io": word_io,
                                "collection_text": collection_text,
                                "mmdd_str": mmdd_str,
                                "yyyymmdd_str": yyyymmdd_str,
                                "has_word": need_report,
                            }
                            st.rerun()
                        else:
            ...
            if cached is not None:
                ...
                with dl_col1:
                    st.download_button(
                        ...
                        data=cached["excel_io"],
                        file_name=f"逾期采购监控表Z_{cached['mmdd_str']}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True
                    )
                if cached["doc_io"]:
                    with dl_col2:
                        st.download_button(
                            ...
                            data=cached["doc_io"],
                            file_name=f"逾期采购报告_{cached['mmdd_str']}.docx",
                            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                            use_container_width=True
                        )
                if cached["web_text"]:
        ...
        # --- 模块 5: 信用风险管理日报 ---
            ...
            if cached is not None:
                ...
                if cached["word_text_dict"]:
                    ...
                    for center_name, content in cached["word_text_dict"].items():
                ...
                st.markdown("### 📥 下载生成文件")
                dl_cols = st.columns(1 + len(cached["export_files"]))
                with dl_cols[0]:
                    if cached["word_bytes"]:
                        st.download_button(
                            label="📄 下载 Word 报告",
                            data=cached["word_bytes"],
                            file_name=f"{cached['uploaded_file_name']}.docx",
                            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                            use_container_width=True
                        )
                for i, export_file in enumerate(cached["export_files"], 1):
                    with dl_cols[i]:
                ...
                png_files = [f for f in cached["export_files"] if f["type"] == "png"]
                if png_files:
                    st.markdown("#### 👁️ 图片预览")
            ...
            if cached is not None:
                ...
                with dl_col1:
                    st.download_button(
                        ...
                        data=cached["excel_io"],
                        file_name=f"2026年中粮贸易预付款业务情况表-{cached['mmdd_str']}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True
                    )
                with dl_col2:
                    st.download_button(
                        ...
                        data=cached["doc_io"],
                        file_name=f"2026年中粮贸易预付款业务情况表-{cached['mmdd_str']}.docx",
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        use_container_width=True
                    )
                if cached["has_summary"]:
